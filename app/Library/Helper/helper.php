...
class helper
{
    public static function setFlashData($status = 'success', $message = 'Successfully!')
    {
        $request = request();
    ...
    public static function translate($key = null, $dataLanguage = [])
    {
        $textTrans = '';
        if ($key && !empty($dataLanguage)) {
            foreach ($dataLanguage as $lang) {
                if ($key == $lang['key']) {
                    $textTrans = $lang['text_translation'];
                    break;
                }
            }
        }
        $textTrans = $textTrans ? $textTrans : $key;
        return $textTrans;
    }
}